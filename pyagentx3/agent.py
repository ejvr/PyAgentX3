...
class Agent:
    ...
    async def _connect(self):
        while True:
            try:
                logger.info("Try to open socket on ({})".format(
                    self._socket_path))
        ...
        while True:
            try:
                ...
                if 'oid' in item:
                    ...
                    trap_pdu.values.append(
                        {'name': Agent.SNMP_TRAP_OID, 'type': pyagentx3.TYPE_OBJECTIDENTIFIER, 'value': item['trap_oid']})
